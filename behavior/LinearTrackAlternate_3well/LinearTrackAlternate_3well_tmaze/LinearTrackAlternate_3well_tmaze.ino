/*
  ...
  by Nat Kinsky
  modified 8 Sep 2021
  by Rachel Wahlberg (for T maze version)
 */
...
int valve1 = 9; // end valve 1
int valve2 = 8; // end valve 2
...
int pumpOpen = 100; // ms, time each valve is open after a lick.
  ...
  if (digitalRead(sensor1)==HIGH && sensorTrig ==0)
{
  ...
  // visits to 1 from middle -> open 1 only
  if (digitalRead(sensor1)==HIGH && (next_well ==1 || next_well==0) && last_well ==3)
{
    digitalWrite(valve1, HIGH);   // turn the LED on (HIGH is the voltage level) at middle well
    digitalWrite(OpenEphys, HIGH);
    delay(pumpOpen);               // wait for a second
    digitalWrite(valve1, LOW);    // turn the LED off by making the voltage LOW
    digitalWrite(OpenEphys, LOW); 
  ...
  if (digitalRead(sensor2)==HIGH && sensorTrig ==0)  
{
  ...
  // visits to 2 from middle -> open 2 only
  ...
  if (digitalRead(sensor3)==HIGH && sensorTrig ==0)  // first well visited is middle
{
  ...
  // visits to middle from end 1 -> open MIDDLE only
  if (digitalRead(sensor3)==HIGH && next_well ==3 && last_well==1 && sensorTrig == 1)
{
  ...
  // visits to middle from 2 -> open MIDDLE only
  if (digitalRead(sensor3)==HIGH && next_well ==3 && last_well==2 && sensorTrig == 1)
{
    digitalWrite(valve3, HIGH);   // turn the LED on (HIGH is the voltage level)
    digitalWrite(OpenEphys, HIGH);
    delay(pumpOpen);               // wait for a second
...
// adding in for the other non-designated well

 if (digitalRead(sensor1)==HIGH && next_well !=3) // && sensorTrig ==1)
{
