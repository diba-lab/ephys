...
def load_openephys(folder, Experiment=1, Recording=1, TTLport=None):
    ...
    try:  # Load in openephys format
        ...
        Proc = list(PluginNames.keys())[
            0
        ]  # assume first node is the recording processor
...
def plot_opti_v_mat(
    ...
    # Get time elapsed
    record_start_time = mat_data["time_mat"][
        np.where(np.bitwise_not(np.isnan(mat_data["trig_on"])))[0][0], :
    ]
    ...
    try:  # Load from on_minutes input variable which tracks TTL pulses for when recording starts and switches every minute
        ...
        on_time_mat = mat_data["time_mat"][
            np.where(np.bitwise_not(np.isnan(mat_data["trig_on"])))
        ]
    ...
    oe_times_aligned = (
        np.arange(cont_data.shape[0]).reshape(cont_data.shape[0], -1) / Rate - on_time
    )
    ...
    for chan, color in zip(LED_chans, plot_colors):
        ...
        [
            ax1[1][2].plot(event_times_aligned[[start, start]], ylims, color)
            for start in starts
        ]
...
def resampleOEtoLFP(traces, SRin=30000, SRout=1250):
    ...
    Note you should only use this in dire circumstances!!! Use 'process_resample' from the ndmanager plugins if
    ...
    if SRin == 30000 and SRout == 1250:
        ...
        for chan in range(0, nchannels):
            ds_trace_list.append(
                signal.decimate(signal.decimate(traces[:, chan], 6), 4)
            )
...
def oe_to_lfp_file(folder, Experiment=1, Recording=1, SReeg=1250):
    ...
    _, full_traces, SRoe = load_openephys(
        folder, Experiment=Experiment, Recording=Recording
    )

    _, PluginNames = SettingsXML.GetRecChs(os.path.join(folder, "settings.xml"))
    Proc = list(PluginNames.keys())[0]  # assume first node is the recording processor
    traces_eeg, _ = resampleOEtoLFP(
        full_traces[Proc][str(Experiment - 1)][str(Recording - 1)],
        SRin=SRoe,
        SRout=1250,
    )
