...
def set_ytick_units(ax, unit):
    ...
    ylims_round = [
        int(np.floor(ax.get_ylim()[0] / unit) * unit),
        int(np.ceil(ax.get_ylim()[1] / unit) * unit),
    ]
...
def mat_time_to_sec(t0, t):
    ...
    for yr, mo, dy, hr, mi, s, ms in zip(year, month, day, hour, min, sec, msec):
        diff_temp = (
            datetime.datetime(
                int(yr), int(mo), int(dy), int(hr), int(mi), int(s), int(ms)
            )
            - t0py
        )
...
def plot_pe_raster(
    ...
    if ax is None:
        fig, ax = plt.subplots()  # Make new figure if no specified
    if event_ends is None:
        event_ends = event_starts  # set up arrays properly
    ...
    for idn, start in enumerate(event_starts):
        spike_times_aligned = (
            spike_times[
                np.bitwise_and(
                    spike_times > (start - buffer),
                    spike_times < (event_ends[idn] + buffer),
                )
            ]
            - start
        )
        ...
        ax.vlines(
            spike_times_aligned, idn, idn + 1, color=spike_color
        )  # plot each row of the raster

    # Add box if specified
    if box_color is not None:
        event_poly = np.concatenate(
            (
                np.stack((durations, np.arange(1, len(durations) + 1)), axis=1),
                np.stack(
                    (np.zeros_like(durations), np.arange(len(durations), 0, -1)), axis=1
                ),
            ),
            axis=0,
        )
...
def opto_boxplot(
    spike_times, on_times, off_times, buffer=None, ax=None, color_palette="Set2"
):
    """
    ...
    if ax is None:
        fig, ax = plt.subplots()  # set up new axes if necessary
    ...
    for idn, (on, off) in enumerate(zip(on_times, off_times)):
        ...
        nbef = np.sum(
            np.bitwise_and(spike_times < on, spike_times > (on - buffer[idn]))
        )
        naft = np.sum(
            np.bitwise_and(spike_times > off, spike_times < (off + buffer[idn]))
        )
